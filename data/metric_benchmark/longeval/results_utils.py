...
from time_utils import extract_worker_stats
from preprocessing_utils import csv_dict_read, jsonl_read
...
def compute_fine_scores(fine_annotation_data, story_list, story_key_fn, skip_model, num_keep, summaries=None, verbose=False):
    all_fine_scores = []

    for story, summary in zip(story_list, summaries):
        ...
        # our intervetion to extract sentence_level labels
        summary_sentences = nltk.sent_tokenize(summary)

        summary_sentence_labels = {k["wid"]:[list() for _ in range(len(summary_sentences))] for k in worker_scores}
        for worker_data in worker_scores:
            sorted_rows = sort_by_timestamps(worker_data["rows"])
            # from sorted row find the relevant sentence
            for row in sorted_rows:
                annotated_summary = row["Input.annotated_summary"]
                # find the span surrounded by <span id='claim-sent' style='background-color: #ffd500'> and </span>
                start = annotated_summary.find("<span id='claim-sent' style='background-color: #ffd500'>")
                end = annotated_summary.find("</span>")
                span = annotated_summary[start+len("<span id='claim-sent' style='background-color: #ffd500'>"):end]
                sent_id = -1
                # find the sentence that contains the span
                for i, sent in enumerate(summary_sentences):
                    if span in sent:
                        sent_id = i
                        break
                summary_sentence_labels[worker_data["wid"]][sent_id].append(int(row["Answer.semantic-similarity.label"] == "Yes"))
        # print(summary_sentence_labels)
        # merge across sentences
        for k, v in summary_sentence_labels.items():
            summary_sentence_labels[k] = [np.min(x) if len(x) > 0 else None for x in v]
        # print(summary_sentence_labels)
        # merge across workers through majority voting
        summary_sentence_finals = [list() for _ in range(len(summary_sentences))]
        for i in range(len(summary_sentences)):
            for k, v in summary_sentence_labels.items():
                if v[i] is not None:
                    summary_sentence_finals[i].append(v[i])
        summary_sentence_finals = [int(np.sum(x) > len(x)//2 ) if len(x) > 0 else None for x in summary_sentence_finals]
        print(summary_sentence_finals)
        # all_y_pts = []
        # for worker_data in worker_scores:
        #     sorted_rows = sort_by_timestamps(worker_data["rows"])
        ...
        all_fine_scores.append(summary_sentence_finals)
